...
async function checkBreakMode() {
    try {
        ...
        if (!res.ok) throw new Error('Błąd pobierania statusu przerwy');
        const data = await res.json();
        breakModeActive = data.isActive;
        
        if (breakModeActive) {
            console.log('[PRZERWA] Tryb przerwy jest WŁĄCZONY');
        }
        
        return breakModeActive;
    } catch (e) {
        console.error('Błąd sprawdzania trybu przerwy:', e);
        return false;
    }
}

function showAdminLoginButton() {
    let adminBtn = document.getElementById('hiddenAdminBtn');
    if (adminBtn) {
    ...
    adminBtn.innerHTML = `
        <button onclick="window.location.href='/auth/discord'" style="
            position: fixed;
            bottom: 30px;
            right: 30px;
        ...
        ">
            ...
            ADMIN - Zaloguj przez Discord
        </button>
    `;
    document.body.appendChild(adminBtn);
    
    adminBtn.querySelector('button').addEventListener('mouseenter', function() {
        this.style.transform = 'scale(1.05)';
        this.style.boxShadow = '0 6px 20px rgba(88, 101, 242, 0.6)';
    });
    adminBtn.querySelector('button').addEventListener('mouseleave', function() {
        this.style.transform = 'scale(1)';
        this.style.boxShadow = '0 4px 15px rgba(88, 101, 242, 0.4)';
    });
}

async function toggleBreakModeFromKeyboard() {
    console.log('[DEBUG] Próba wyłączenia przerwy...');
    try {
        const res = await fetch('/api/break-status');
        if (!res.ok) throw new Error('Błąd pobierania statusu przerwy');
        const data = await res.json();
        console.log('[DEBUG] Status przerwy:', data);
        const currentlyActive = data.isActive;
        
        const newState = !currentlyActive;
        console.log('[DEBUG] Nowy stan przerwy:', newState);
        
        const saveRes = await fetch('/api/break-mode', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ isActive: newState, message: '' })
        });
        
        if (!saveRes.ok) throw new Error('Błąd zapisu trybu przerwy');
        const saveData = await saveRes.json();
        console.log('[DEBUG] Wynik zapisu:', saveData);
        
        breakModeActive = newState;
        console.log(`[PRZERWA] Tryb przerwy: ${newState ? 'WŁĄCZONY' : 'WYŁĄCZONY'}`);
        alert(`Przerwa została ${newState ? 'WŁĄCZONA' : 'WYŁĄCZONA'}`);
        return newState;
    } catch (e) {
        console.error('[DEBUG] Błąd przełączania przerwy:', e);
...
document.addEventListener('keydown', async function(e) {
    ...
    console.log('[DEBUG] Aktualny kod:', secretCode);
    
    if (secretCode.includes('stopprzerwa01')) {
        console.log('[DEBUG] Wykryto stopprzerwa01!');
        await toggleBreakModeFromKeyboard();
        showAdminPanel();
        secretCode = '';
    }
    
    if (secretCode.includes('killer02')) {
        await checkBreakMode();
        if (!breakModeActive) {
            showAdminPanel();
        }
        secretCode = '';
    ...
    if (secretCode.includes('armor topka')) {
        await checkBreakMode();
        if (breakModeActive) {
